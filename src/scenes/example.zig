...
pub fn ExampleScene(comptime Theme: type) type {
    ...
    return struct {
        ...
        pub fn draw(self: *Self, mouse: Mouse, dt: f32, renderer: *Render) void {
            self.begin_frame(renderer);

            if (self.ui_visible) self.action_state.update();

            self.update_simulation(mouse, dt, renderer);
            self.render_world(renderer, dt);

            self.handle_top_controls(mouse, renderer);
            self.handle_ui_actions(mouse, renderer);
            self.render_ui(renderer);
        }

        fn begin_frame(self: *Self, renderer: *Render) void {
            renderer.copy_buffer(.terrain, .frame);
            renderer.set_target(.frame);
            _ = self;
        }

        fn update_simulation(self: *Self, mouse: Mouse, dt: f32, renderer: *Render) void {
        ...
        fn render_world(self: *Self, renderer: *Render, dt: f32) void {
        ...
        fn handle_ui_actions(self: *Self, mouse: Mouse, renderer: *Render) void {
        ...
        fn spawn_random_sprite(self: *Self) !void {
            const def = &self.sprites_defs[ANIMATED_DEF_INDEX];
        ...
        fn init_terrain(self: *Self, renderer: *Render) void {
            ...
            const terrain_def = &self.sprites_defs[TERRAIN_DEF_INDEX];
